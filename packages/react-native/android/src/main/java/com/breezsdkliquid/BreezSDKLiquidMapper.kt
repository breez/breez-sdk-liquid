...
fun asAcceptPaymentProposedFeesRequest(acceptPaymentProposedFeesRequest: ReadableMap): AcceptPaymentProposedFeesRequest? {
    if (!validateMandatoryFields(acceptPaymentProposedFeesRequest, arrayOf(
            "response",
    ))) {
        return null
    }
...
fun asAesSuccessActionData(aesSuccessActionData: ReadableMap): AesSuccessActionData? {
    if (!validateMandatoryFields(aesSuccessActionData, arrayOf(
            "description",
            "ciphertext",
            "iv",
    ))) {
...
fun asAesSuccessActionDataDecrypted(aesSuccessActionDataDecrypted: ReadableMap): AesSuccessActionDataDecrypted? {
    if (!validateMandatoryFields(aesSuccessActionDataDecrypted, arrayOf(
            "description",
            "plaintext",
    ))) {
        return null
...
fun asAssetBalance(assetBalance: ReadableMap): AssetBalance? {
    if (!validateMandatoryFields(assetBalance, arrayOf(
            "assetId",
            "balanceSat",
    ))) {
        return null
...
fun asAssetInfo(assetInfo: ReadableMap): AssetInfo? {
    if (!validateMandatoryFields(assetInfo, arrayOf(
            "name",
            "ticker",
            "amount",
    ))) {
...
fun asAssetMetadata(assetMetadata: ReadableMap): AssetMetadata? {
    if (!validateMandatoryFields(assetMetadata, arrayOf(
            "assetId",
            "name",
            "ticker",
            "precision",
...
fun asBackupRequest(backupRequest: ReadableMap): BackupRequest? {
    if (!validateMandatoryFields(backupRequest, arrayOf(
    ))) {
        return null
    }
...
fun asBitcoinAddressData(bitcoinAddressData: ReadableMap): BitcoinAddressData? {
    if (!validateMandatoryFields(bitcoinAddressData, arrayOf(
            "address",
            "network",
    ))) {
        return null
...
fun asBlockchainInfo(blockchainInfo: ReadableMap): BlockchainInfo? {
    if (!validateMandatoryFields(blockchainInfo, arrayOf(
            "liquidTip",
            "bitcoinTip",
    ))) {
        return null
...
fun asBuyBitcoinRequest(buyBitcoinRequest: ReadableMap): BuyBitcoinRequest? {
    if (!validateMandatoryFields(buyBitcoinRequest, arrayOf(
            "prepareResponse",
    ))) {
        return null
    }
...
fun asCheckMessageRequest(checkMessageRequest: ReadableMap): CheckMessageRequest? {
    if (!validateMandatoryFields(checkMessageRequest, arrayOf(
            "message",
            "pubkey",
            "signature",
    ))) {
...
fun asCheckMessageResponse(checkMessageResponse: ReadableMap): CheckMessageResponse? {
    if (!validateMandatoryFields(checkMessageResponse, arrayOf(
            "isValid",
    ))) {
        return null
    }
...
fun asConfig(config: ReadableMap): Config? {
    if (!validateMandatoryFields(config, arrayOf(
            "liquidExplorer",
            "bitcoinExplorer",
            "workingDir",
            "network",
    ...
    val zeroConfMaxAmountSat = if (hasNonNullKey(config, "zeroConfMaxAmountSat")) config.getDouble("zeroConfMaxAmountSat").toULong() else null
    ...
    val externalInputParsers = if (hasNonNullKey(config, "externalInputParsers")) config.getArray("externalInputParsers")?.let { asExternalInputParserList(it) } else null
    val onchainFeeRateLeewaySat = if (hasNonNullKey(config, "onchainFeeRateLeewaySat")) config.getDouble("onchainFeeRateLeewaySat").toULong() else null
    val assetMetadata = if (hasNonNullKey(config, "assetMetadata")) config.getArray("assetMetadata")?.let { asAssetMetadataList(it) } else null
    val sideswapApiKey = if (hasNonNullKey(config, "sideswapApiKey")) config.getString("sideswapApiKey") else null
    return Config(liquidExplorer,bitcoinExplorer,workingDir,network,paymentTimeoutSec,syncServiceUrl,breezApiKey,zeroConfMaxAmountSat,useDefaultExternalInputParsers,useMagicRoutingHints,externalInputParsers,onchainFeeRateLeewaySat,assetMetadata,sideswapApiKey,)    
}
...
fun asConnectRequest(connectRequest: ReadableMap): ConnectRequest? {
    if (!validateMandatoryFields(connectRequest, arrayOf(
            "config",
    ))) {
        return null
    }
...
fun asConnectWithSignerRequest(connectWithSignerRequest: ReadableMap): ConnectWithSignerRequest? {
    if (!validateMandatoryFields(connectWithSignerRequest, arrayOf(
            "config",
    ))) {
        return null
    }
...
fun asCreateBolt12InvoiceRequest(createBolt12InvoiceRequest: ReadableMap): CreateBolt12InvoiceRequest? {
    if (!validateMandatoryFields(createBolt12InvoiceRequest, arrayOf(
            "offer",
            "invoiceRequest",
    ))) {
        return null
...
fun asCreateBolt12InvoiceResponse(createBolt12InvoiceResponse: ReadableMap): CreateBolt12InvoiceResponse? {
    if (!validateMandatoryFields(createBolt12InvoiceResponse, arrayOf(
            "invoice",
    ))) {
        return null
    }
...
fun asCurrencyInfo(currencyInfo: ReadableMap): CurrencyInfo? {
    if (!validateMandatoryFields(currencyInfo, arrayOf(
            "name",
            "fractionSize",
            "localizedName",
            "localeOverrides",
...
fun asExternalInputParser(externalInputParser: ReadableMap): ExternalInputParser? {
    if (!validateMandatoryFields(externalInputParser, arrayOf(
            "providerId",
            "inputRegex",
            "parserUrl",
    ))) {
...
fun asFetchPaymentProposedFeesRequest(fetchPaymentProposedFeesRequest: ReadableMap): FetchPaymentProposedFeesRequest? {
    if (!validateMandatoryFields(fetchPaymentProposedFeesRequest, arrayOf(
            "swapId",
    ))) {
        return null
    }
...
fun asFetchPaymentProposedFeesResponse(fetchPaymentProposedFeesResponse: ReadableMap): FetchPaymentProposedFeesResponse? {
    if (!validateMandatoryFields(fetchPaymentProposedFeesResponse, arrayOf(
            "swapId",
            "feesSat",
            "payerAmountSat",
            "receiverAmountSat",
...
fun asFiatCurrency(fiatCurrency: ReadableMap): FiatCurrency? {
    if (!validateMandatoryFields(fiatCurrency, arrayOf(
            "id",
            "info",
    ))) {
        return null
...
fun asGetInfoResponse(getInfoResponse: ReadableMap): GetInfoResponse? {
    if (!validateMandatoryFields(getInfoResponse, arrayOf(
            "walletInfo",
            "blockchainInfo",
    ))) {
        return null
...
fun asLnInvoice(lnInvoice: ReadableMap): LnInvoice? {
    if (!validateMandatoryFields(lnInvoice, arrayOf(
            "bolt11",
            "network",
            "payeePubkey",
            "paymentHash",
    ...
    return LnInvoice(bolt11,network,payeePubkey,paymentHash,description,descriptionHash,amountMsat,timestamp,expiry,routingHints,paymentSecret,minFinalCltvExpiryDelta,)    
}
...
fun asLnOffer(lnOffer: ReadableMap): LnOffer? {
    if (!validateMandatoryFields(lnOffer, arrayOf(
            "offer",
            "chains",
            "paths",
    ))) {
...
fun asLightningPaymentLimitsResponse(lightningPaymentLimitsResponse: ReadableMap): LightningPaymentLimitsResponse? {
    if (!validateMandatoryFields(lightningPaymentLimitsResponse, arrayOf(
            "send",
            "receive",
    ))) {
        return null
...
fun asLimits(limits: ReadableMap): Limits? {
    if (!validateMandatoryFields(limits, arrayOf(
            "minSat",
            "maxSat",
            "maxZeroConfSat",
    ))) {
...
fun asLiquidAddressData(liquidAddressData: ReadableMap): LiquidAddressData? {
    if (!validateMandatoryFields(liquidAddressData, arrayOf(
            "address",
            "network",
    ))) {
        return null
...
fun asListPaymentsRequest(listPaymentsRequest: ReadableMap): ListPaymentsRequest? {
    if (!validateMandatoryFields(listPaymentsRequest, arrayOf(
    ))) {
        return null
    }
    val filters = if (hasNonNullKey(listPaymentsRequest, "filters")) listPaymentsRequest.getArray("filters")?.let { asPaymentTypeList(it) } else null
    val states = if (hasNonNullKey(listPaymentsRequest, "states")) listPaymentsRequest.getArray("states")?.let { asPaymentStateList(it) } else null
    val fromTimestamp = if (hasNonNullKey(listPaymentsRequest, "fromTimestamp")) listPaymentsRequest.getDouble("fromTimestamp").toLong() else null
    ...
    val details = if (hasNonNullKey(listPaymentsRequest, "details")) listPaymentsRequest.getMap("details")?.let { asListPaymentDetails(it)} else null
...
fun asLnOfferBlindedPath(lnOfferBlindedPath: ReadableMap): LnOfferBlindedPath? {
    if (!validateMandatoryFields(lnOfferBlindedPath, arrayOf(
            "blindedHops",
    ))) {
        return null
    }
...
fun asLnUrlAuthRequestData(lnUrlAuthRequestData: ReadableMap): LnUrlAuthRequestData? {
    if (!validateMandatoryFields(lnUrlAuthRequestData, arrayOf(
            "k1",
            "domain",
            "url",
    ))) {
...
fun asLnUrlErrorData(lnUrlErrorData: ReadableMap): LnUrlErrorData? {
    if (!validateMandatoryFields(lnUrlErrorData, arrayOf(
            "reason",
    ))) {
        return null
    }
...
fun asLnUrlInfo(lnUrlInfo: ReadableMap): LnUrlInfo? {
    if (!validateMandatoryFields(lnUrlInfo, arrayOf(
    ))) {
        return null
    }
    ...
    val lnurlPaySuccessAction = if (hasNonNullKey(lnUrlInfo, "lnurlPaySuccessAction")) lnUrlInfo.getMap("lnurlPaySuccessAction")?.let { asSuccessActionProcessed(it)} else null
    val lnurlPayUnprocessedSuccessAction = if (hasNonNullKey(lnUrlInfo, "lnurlPayUnprocessedSuccessAction")) lnUrlInfo.getMap("lnurlPayUnprocessedSuccessAction")?.let { asSuccessAction(it)} else null
    val lnurlWithdrawEndpoint = if (hasNonNullKey(lnUrlInfo, "lnurlWithdrawEndpoint")) lnUrlInfo.getString("lnurlWithdrawEndpoint") else null
    return LnUrlInfo(lnAddress,lnurlPayComment,lnurlPayDomain,lnurlPayMetadata,lnurlPaySuccessAction,lnurlPayUnprocessedSuccessAction,lnurlWithdrawEndpoint,)    
}
...
fun asLnUrlPayErrorData(lnUrlPayErrorData: ReadableMap): LnUrlPayErrorData? {
    if (!validateMandatoryFields(lnUrlPayErrorData, arrayOf(
            "paymentHash",
            "reason",
    ))) {
        return null
...
fun asLnUrlPayRequest(lnUrlPayRequest: ReadableMap): LnUrlPayRequest? {
    if (!validateMandatoryFields(lnUrlPayRequest, arrayOf(
            "prepareResponse",
    ))) {
        return null
    }
...
fun asLnUrlPayRequestData(lnUrlPayRequestData: ReadableMap): LnUrlPayRequestData? {
    if (!validateMandatoryFields(lnUrlPayRequestData, arrayOf(
            "callback",
            "minSendable",
            "maxSendable",
            "metadataStr",
    ...
    return LnUrlPayRequestData(callback,minSendable,maxSendable,metadataStr,commentAllowed,domain,allowsNostr,nostrPubkey,lnAddress,)    
}
...
fun asLnUrlPaySuccessData(lnUrlPaySuccessData: ReadableMap): LnUrlPaySuccessData? {
    if (!validateMandatoryFields(lnUrlPaySuccessData, arrayOf(
            "payment",
    ))) {
        return null
    }
    val successAction = if (hasNonNullKey(lnUrlPaySuccessData, "successAction")) lnUrlPaySuccessData.getMap("successAction")?.let { asSuccessActionProcessed(it)} else null
...
fun asLnUrlWithdrawRequest(lnUrlWithdrawRequest: ReadableMap): LnUrlWithdrawRequest? {
    if (!validateMandatoryFields(lnUrlWithdrawRequest, arrayOf(
            "data",
            "amountMsat",
    ))) {
        return null
...
fun asLnUrlWithdrawRequestData(lnUrlWithdrawRequestData: ReadableMap): LnUrlWithdrawRequestData? {
    if (!validateMandatoryFields(lnUrlWithdrawRequestData, arrayOf(
            "callback",
            "k1",
            "defaultDescription",
            "minWithdrawable",
...
fun asLnUrlWithdrawSuccessData(lnUrlWithdrawSuccessData: ReadableMap): LnUrlWithdrawSuccessData? {
    if (!validateMandatoryFields(lnUrlWithdrawSuccessData, arrayOf(
            "invoice",
    ))) {
        return null
    }
...
fun asLocaleOverrides(localeOverrides: ReadableMap): LocaleOverrides? {
    if (!validateMandatoryFields(localeOverrides, arrayOf(
            "locale",
            "symbol",
    ))) {
        return null
...
fun asLocalizedName(localizedName: ReadableMap): LocalizedName? {
    if (!validateMandatoryFields(localizedName, arrayOf(
            "locale",
            "name",
    ))) {
        return null
...
fun asLogEntry(logEntry: ReadableMap): LogEntry? {
    if (!validateMandatoryFields(logEntry, arrayOf(
            "line",
            "level",
    ))) {
        return null
...
fun asMessageSuccessActionData(messageSuccessActionData: ReadableMap): MessageSuccessActionData? {
    if (!validateMandatoryFields(messageSuccessActionData, arrayOf(
            "message",
    ))) {
        return null
    }
...
fun asOnchainPaymentLimitsResponse(onchainPaymentLimitsResponse: ReadableMap): OnchainPaymentLimitsResponse? {
    if (!validateMandatoryFields(onchainPaymentLimitsResponse, arrayOf(
            "send",
            "receive",
    ))) {
        return null
...
fun asPayOnchainRequest(payOnchainRequest: ReadableMap): PayOnchainRequest? {
    if (!validateMandatoryFields(payOnchainRequest, arrayOf(
            "address",
            "prepareResponse",
    ))) {
        return null
...
fun asPayment(payment: ReadableMap): Payment? {
    if (!validateMandatoryFields(payment, arrayOf(
            "timestamp",
            "amountSat",
            "feesSat",
            "paymentType",
...
fun asPrepareBuyBitcoinRequest(prepareBuyBitcoinRequest: ReadableMap): PrepareBuyBitcoinRequest? {
    if (!validateMandatoryFields(prepareBuyBitcoinRequest, arrayOf(
            "provider",
            "amountSat",
    ))) {
        return null
...
fun asPrepareBuyBitcoinResponse(prepareBuyBitcoinResponse: ReadableMap): PrepareBuyBitcoinResponse? {
    if (!validateMandatoryFields(prepareBuyBitcoinResponse, arrayOf(
            "provider",
            "amountSat",
            "feesSat",
    ))) {
...
fun asPrepareLnUrlPayRequest(prepareLnUrlPayRequest: ReadableMap): PrepareLnUrlPayRequest? {
    if (!validateMandatoryFields(prepareLnUrlPayRequest, arrayOf(
            "data",
            "amount",
    ))) {
        return null
    ...
    val bip353Address = if (hasNonNullKey(prepareLnUrlPayRequest, "bip353Address")) prepareLnUrlPayRequest.getString("bip353Address") else null
    val comment = if (hasNonNullKey(prepareLnUrlPayRequest, "comment")) prepareLnUrlPayRequest.getString("comment") else null
    val validateSuccessActionUrl = if (hasNonNullKey(prepareLnUrlPayRequest, "validateSuccessActionUrl")) prepareLnUrlPayRequest.getBoolean("validateSuccessActionUrl") else null
...
fun asPrepareLnUrlPayResponse(prepareLnUrlPayResponse: ReadableMap): PrepareLnUrlPayResponse? {
    if (!validateMandatoryFields(prepareLnUrlPayResponse, arrayOf(
            "destination",
            "feesSat",
            "data",
            "amount",
    ...
    val successAction = if (hasNonNullKey(prepareLnUrlPayResponse, "successAction")) prepareLnUrlPayResponse.getMap("successAction")?.let { asSuccessAction(it)} else null
...
fun asPreparePayOnchainRequest(preparePayOnchainRequest: ReadableMap): PreparePayOnchainRequest? {
    if (!validateMandatoryFields(preparePayOnchainRequest, arrayOf(
            "amount",
    ))) {
        return null
    }
    val amount = preparePayOnchainRequest.getMap("amount")?.let { asPayAmount(it)}!!
    val feeRateSatPerVbyte = if (hasNonNullKey(preparePayOnchainRequest, "feeRateSatPerVbyte")) preparePayOnchainRequest.getInt("feeRateSatPerVbyte").toUInt() else null
...
fun asPreparePayOnchainResponse(preparePayOnchainResponse: ReadableMap): PreparePayOnchainResponse? {
    if (!validateMandatoryFields(preparePayOnchainResponse, arrayOf(
            "receiverAmountSat",
            "claimFeesSat",
            "totalFeesSat",
    ))) {
...
fun asPrepareReceiveRequest(prepareReceiveRequest: ReadableMap): PrepareReceiveRequest? {
    if (!validateMandatoryFields(prepareReceiveRequest, arrayOf(
            "paymentMethod",
    ))) {
        return null
    }
    val paymentMethod = prepareReceiveRequest.getString("paymentMethod")?.let { asPaymentMethod(it)}!!
    val amount = if (hasNonNullKey(prepareReceiveRequest, "amount")) prepareReceiveRequest.getMap("amount")?.let { asReceiveAmount(it)} else null
...
fun asPrepareReceiveResponse(prepareReceiveResponse: ReadableMap): PrepareReceiveResponse? {
    if (!validateMandatoryFields(prepareReceiveResponse, arrayOf(
            "paymentMethod",
            "feesSat",
    ))) {
        return null
    ...
    val amount = if (hasNonNullKey(prepareReceiveResponse, "amount")) prepareReceiveResponse.getMap("amount")?.let { asReceiveAmount(it)} else null
    val minPayerAmountSat = if (hasNonNullKey(prepareReceiveResponse, "minPayerAmountSat")) prepareReceiveResponse.getDouble("minPayerAmountSat").toULong() else null
    val maxPayerAmountSat = if (hasNonNullKey(prepareReceiveResponse, "maxPayerAmountSat")) prepareReceiveResponse.getDouble("maxPayerAmountSat").toULong() else null
    val swapperFeerate = if (hasNonNullKey(prepareReceiveResponse, "swapperFeerate")) prepareReceiveResponse.getDouble("swapperFeerate") else null
...
fun asPrepareRefundRequest(prepareRefundRequest: ReadableMap): PrepareRefundRequest? {
    if (!validateMandatoryFields(prepareRefundRequest, arrayOf(
            "swapAddress",
            "refundAddress",
            "feeRateSatPerVbyte",
    ))) {
...
fun asPrepareRefundResponse(prepareRefundResponse: ReadableMap): PrepareRefundResponse? {
    if (!validateMandatoryFields(prepareRefundResponse, arrayOf(
            "txVsize",
            "txFeeSat",
    ))) {
        return null
    ...
    val lastRefundTxId = if (hasNonNullKey(prepareRefundResponse, "lastRefundTxId")) prepareRefundResponse.getString("lastRefundTxId") else null
...
fun asPrepareSendRequest(prepareSendRequest: ReadableMap): PrepareSendRequest? {
    if (!validateMandatoryFields(prepareSendRequest, arrayOf(
            "destination",
    ))) {
        return null
    }
    ...
    val disableMrh = if (hasNonNullKey(prepareSendRequest, "disableMrh")) prepareSendRequest.getBoolean("disableMrh") else null
    return PrepareSendRequest(destination,amount,disableMrh,)    
}
...
fun readableMapOf(prepareSendRequest: PrepareSendRequest): ReadableMap {
    return readableMapOf(
            ...
            "disableMrh" to prepareSendRequest.disableMrh,       
    )
}
...
fun asPrepareSendResponse(prepareSendResponse: ReadableMap): PrepareSendResponse? {
    if (!validateMandatoryFields(prepareSendResponse, arrayOf(
            "destination",
    ))) {
        return null
    }
    ...
    val estimatedAssetFees = if (hasNonNullKey(prepareSendResponse, "estimatedAssetFees")) prepareSendResponse.getDouble("estimatedAssetFees") else null
    val exchangeAmountSat = if (hasNonNullKey(prepareSendResponse, "exchangeAmountSat")) prepareSendResponse.getDouble("exchangeAmountSat").toULong() else null
...
fun asRate(rate: ReadableMap): Rate? {
    if (!validateMandatoryFields(rate, arrayOf(
            "coin",
            "value",
    ))) {
        return null
...
fun asReceivePaymentRequest(receivePaymentRequest: ReadableMap): ReceivePaymentRequest? {
    if (!validateMandatoryFields(receivePaymentRequest, arrayOf(
            "prepareResponse",
    ))) {
        return null
    }
    ...
    val useDescriptionHash = if (hasNonNullKey(receivePaymentRequest, "useDescriptionHash")) receivePaymentRequest.getBoolean("useDescriptionHash") else null
...
fun asReceivePaymentResponse(receivePaymentResponse: ReadableMap): ReceivePaymentResponse? {
    if (!validateMandatoryFields(receivePaymentResponse, arrayOf(
            "destination",
    ))) {
        return null
    }
...
fun asRecommendedFees(recommendedFees: ReadableMap): RecommendedFees? {
    if (!validateMandatoryFields(recommendedFees, arrayOf(
            "fastestFee",
            "halfHourFee",
            "hourFee",
            "economyFee",
...
fun asRefundRequest(refundRequest: ReadableMap): RefundRequest? {
    if (!validateMandatoryFields(refundRequest, arrayOf(
            "swapAddress",
            "refundAddress",
            "feeRateSatPerVbyte",
    ))) {
...
fun asRefundResponse(refundResponse: ReadableMap): RefundResponse? {
    if (!validateMandatoryFields(refundResponse, arrayOf(
            "refundTxId",
    ))) {
        return null
    }
...
fun asRefundableSwap(refundableSwap: ReadableMap): RefundableSwap? {
    if (!validateMandatoryFields(refundableSwap, arrayOf(
            "swapAddress",
            "timestamp",
            "amountSat",
    ))) {
...
fun asRestoreRequest(restoreRequest: ReadableMap): RestoreRequest? {
    if (!validateMandatoryFields(restoreRequest, arrayOf(
    ))) {
        return null
    }
...
fun asRouteHint(routeHint: ReadableMap): RouteHint? {
    if (!validateMandatoryFields(routeHint, arrayOf(
            "hops",
    ))) {
        return null
    }
...
fun asRouteHintHop(routeHintHop: ReadableMap): RouteHintHop? {
    if (!validateMandatoryFields(routeHintHop, arrayOf(
            "srcNodeId",
            "shortChannelId",
            "feesBaseMsat",
    ...
    return RouteHintHop(srcNodeId,shortChannelId,feesBaseMsat,feesProportionalMillionths,cltvExpiryDelta,htlcMinimumMsat,htlcMaximumMsat,)    
}
...
fun asSendPaymentRequest(sendPaymentRequest: ReadableMap): SendPaymentRequest? {
    if (!validateMandatoryFields(sendPaymentRequest, arrayOf(
            "prepareResponse",
    ))) {
        return null
    }
    ...
    val disableMrh = if (hasNonNullKey(sendPaymentRequest, "disableMrh")) sendPaymentRequest.getBoolean("disableMrh") else null
    return SendPaymentRequest(prepareResponse,useAssetFees,payerNote,disableMrh,)    
}
...
fun readableMapOf(sendPaymentRequest: SendPaymentRequest): ReadableMap {
    return readableMapOf(
            ...
            "disableMrh" to sendPaymentRequest.disableMrh,       
    )
}
...
fun asSendPaymentResponse(sendPaymentResponse: ReadableMap): SendPaymentResponse? {
    if (!validateMandatoryFields(sendPaymentResponse, arrayOf(
            "payment",
    ))) {
        return null
    }
...
fun asSignMessageRequest(signMessageRequest: ReadableMap): SignMessageRequest? {
    if (!validateMandatoryFields(signMessageRequest, arrayOf(
            "message",
    ))) {
        return null
    }
...
fun asSignMessageResponse(signMessageResponse: ReadableMap): SignMessageResponse? {
    if (!validateMandatoryFields(signMessageResponse, arrayOf(
            "signature",
    ))) {
        return null
    }
...
fun asSymbol(symbol: ReadableMap): Symbol? {
    if (!validateMandatoryFields(symbol, arrayOf(
    ))) {
        return null
    }
...
fun asUrlSuccessActionData(urlSuccessActionData: ReadableMap): UrlSuccessActionData? {
    if (!validateMandatoryFields(urlSuccessActionData, arrayOf(
            "description",
            "url",
...
fun asWalletInfo(walletInfo: ReadableMap): WalletInfo? {
    if (!validateMandatoryFields(walletInfo, arrayOf(
            "balanceSat",
            "pendingSendSat",
            "pendingReceiveSat",
            "fingerprint",
...
fun asLiquidNetwork(type: String): LiquidNetwork {
    return LiquidNetwork.valueOf(camelToUpperSnakeCase(type))
}
...
fun asPaymentDetails(paymentDetails: ReadableMap): PaymentDetails? {
    ...
    if (type == "lightning") {
            ...
            val destinationPubkey = if (hasNonNullKey(paymentDetails, "destinationPubkey")) paymentDetails.getString("destinationPubkey") else null
            val lnurlInfo = if (hasNonNullKey(paymentDetails, "lnurlInfo")) paymentDetails.getMap("lnurlInfo")?.let { asLnUrlInfo(it)} else null
            ...
            val refundTxAmountSat = if (hasNonNullKey(paymentDetails, "refundTxAmountSat")) paymentDetails.getDouble("refundTxAmountSat").toULong() else null
            return PaymentDetails.Lightning(swapId,description,liquidExpirationBlockheight,preimage,invoice,bolt12Offer,paymentHash,destinationPubkey,lnurlInfo,bip353Address,payerNote,claimTxId,refundTxId,refundTxAmountSat,)       
        }        
    if (type == "liquid") {
            ...
            val description = paymentDetails.getString("description")!!
            val assetInfo = if (hasNonNullKey(paymentDetails, "assetInfo")) paymentDetails.getMap("assetInfo")?.let { asAssetInfo(it)} else null
            val lnurlInfo = if (hasNonNullKey(paymentDetails, "lnurlInfo")) paymentDetails.getMap("lnurlInfo")?.let { asLnUrlInfo(it)} else null
            val bip353Address = if (hasNonNullKey(paymentDetails, "bip353Address")) paymentDetails.getString("bip353Address") else null
            val payerNote = if (hasNonNullKey(paymentDetails, "payerNote")) paymentDetails.getString("payerNote") else null
    ...
    if (type == "bitcoin") {
            ...
            val bitcoinExpirationBlockheight = if (hasNonNullKey(paymentDetails, "bitcoinExpirationBlockheight")) paymentDetails.getInt("bitcoinExpirationBlockheight").toUInt() else null
            val liquidExpirationBlockheight = if (hasNonNullKey(paymentDetails, "liquidExpirationBlockheight")) paymentDetails.getInt("liquidExpirationBlockheight").toUInt() else null
            val lockupTxId = if (hasNonNullKey(paymentDetails, "lockupTxId")) paymentDetails.getString("lockupTxId") else null
            val claimTxId = if (hasNonNullKey(paymentDetails, "claimTxId")) paymentDetails.getString("claimTxId") else null
            val refundTxId = if (hasNonNullKey(paymentDetails, "refundTxId")) paymentDetails.getString("refundTxId") else null
            val refundTxAmountSat = if (hasNonNullKey(paymentDetails, "refundTxAmountSat")) paymentDetails.getDouble("refundTxAmountSat").toULong() else null
            return PaymentDetails.Bitcoin(swapId,bitcoinAddress,description,autoAcceptedFees,bitcoinExpirationBlockheight,liquidExpirationBlockheight,lockupTxId,claimTxId,refundTxId,refundTxAmountSat,)       
        }        
    return null
}
...
fun asPaymentMethod(type: String): PaymentMethod {
    return PaymentMethod.valueOf(camelToUpperSnakeCase(type))
}
