...
fun asConfig(config: ReadableMap): Config? {
    ...
    return Config(liquidExplorer,bitcoinExplorer,workingDir,network,paymentTimeoutSec,syncServiceUrl,breezApiKey,zeroConfMaxAmountSat,useDefaultExternalInputParsers,useMagicRoutingHints,externalInputParsers,onchainFeeRateLeewaySat,assetMetadata,sideswapApiKey,nwcOptions,)    
}
...
fun asNwcOptions(nwcOptions: ReadableMap): NwcOptions? {
    if (!validateMandatoryFields(nwcOptions, arrayOf(
            "enabled",
    ))) {
        return null
    }
...
fun asSdkEvent(sdkEvent: ReadableMap): SdkEvent? {
    ...
    if (type == "nwc") {
            ...
            return SdkEvent.NWC(details,eventId,)       
        }        
    return null
}
