...
fun asConfig(config: ReadableMap): Config? {
    ...
    val onchainFeeRateLeewaySatPerVbyte = if (hasNonNullKey(config, "onchainFeeRateLeewaySatPerVbyte")) config.getInt("onchainFeeRateLeewaySatPerVbyte").toUInt() else null
    ...
    return Config(liquidExplorers,bitcoinExplorers,workingDir,network,paymentTimeoutSec,syncServiceUrl,breezApiKey,cacheDir,zeroConfMaxAmountSat,useDefaultExternalInputParsers,externalInputParsers,onchainFeeRateLeewaySatPerVbyte,assetMetadata,)    
}
...
fun asPaymentDetails(paymentDetails: ReadableMap): PaymentDetails? {
    ...
    if (type == "lightning") {
            ...
            return PaymentDetails.Lightning(swapId,description,liquidExpirationBlockheight,preimage,invoice,bolt12Offer,paymentHash,destinationPubkey,lnurlInfo,bip353Address,claimTxId,refundTxId,refundTxAmountSat,)       
        }        
    if (type == "liquid") {
    ...
    if (type == "bitcoin") {
            ...
            val refundTxId = if (hasNonNullKey(paymentDetails, "refundTxId")) paymentDetails.getString("refundTxId") else null
            val refundTxAmountSat = if (hasNonNullKey(paymentDetails, "refundTxAmountSat")) paymentDetails.getDouble("refundTxAmountSat").toULong() else null
            return PaymentDetails.Bitcoin(swapId,description,autoAcceptedFees,bitcoinExpirationBlockheight,liquidExpirationBlockheight,claimTxId,refundTxId,refundTxAmountSat,)       
        }        
    return null
}
