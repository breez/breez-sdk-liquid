...
fun asPayAmount(payAmount: ReadableMap): PayAmount? {
    ...
    if (type == "asset") {
        ...
        val useWalletFunds = if (hasNonNullKey(payAmount, "useWalletFunds")) payAmount.getBoolean("useWalletFunds") else null
        return PayAmount.Asset(assetId, receiverAmount, estimateAssetFees, useWalletFunds)
    }
    if (type == "drain") {
...
fun readableMapOf(payAmount: PayAmount): ReadableMap? {
    ...
    when (payAmount) {
        ...
        is PayAmount.Asset -> {
            ...
            pushToMap(map, "useWalletFunds", payAmount.useWalletFunds)
        }
        is PayAmount.Drain -> {
