...
static func asPrepareSendRequest(prepareSendRequest: [String: Any?]) throws -> PrepareSendRequest {
    guard let destination = prepareSendRequest["destination"] as? String else {
        ...
        var disableMrh: Bool?
        if hasNonNilKey(data: prepareSendRequest, key: "disableMrh") {
            guard let disableMrhTmp = prepareSendRequest["disableMrh"] as? Bool else {
                throw SdkError.Generic(message: errUnexpectedValue(fieldName: "disableMrh"))
            }
            disableMrh = disableMrhTmp
        }
    
    return PrepareSendRequest(destination: destination, amount: amount, disableMrh: disableMrh)    
}
...
static func  dictionaryOf(prepareSendRequest: PrepareSendRequest) -> [String: Any?] {
    return [
            ...
            "disableMrh": prepareSendRequest.disableMrh == nil ? nil : prepareSendRequest.disableMrh,       
    ]
}
...
static func asSendPaymentRequest(sendPaymentRequest: [String: Any?]) throws -> SendPaymentRequest {
    ...
    let prepareResponse = try asPrepareSendResponse(prepareSendResponse: prepareResponseTmp)
        ...
        var disableMrh: Bool?
        if hasNonNilKey(data: sendPaymentRequest, key: "disableMrh") {
            guard let disableMrhTmp = sendPaymentRequest["disableMrh"] as? Bool else {
                throw SdkError.Generic(message: errUnexpectedValue(fieldName: "disableMrh"))
            }
            disableMrh = disableMrhTmp
        }
    
    return SendPaymentRequest(prepareResponse: prepareResponse, useAssetFees: useAssetFees, payerNote: payerNote, disableMrh: disableMrh)    
}
...
static func  dictionaryOf(sendPaymentRequest: SendPaymentRequest) -> [String: Any?] {
    return [
            ...
            "disableMrh": sendPaymentRequest.disableMrh == nil ? nil : sendPaymentRequest.disableMrh,       
    ]
}
...
static func asNwcEvent(nwcEvent: [String: Any?]) throws -> NwcEvent {
    let type = nwcEvent["type"] as! String
        ...
        if (type == "payInvoiceHandled") {
            ...
            return NwcEvent.payInvoiceHandled(success: _success, preimage: _preimage, feesSat: _feesSat, error: _error)       
        }
...
static func dictionaryOf(nwcEvent: NwcEvent) -> [String: Any?] {    
    ...
    return [
            "type": "connectedHandled",
        ]
    
    case .disconnectedHandled:  
    return [
            "type": "disconnectedHandled",
        ]
    
    case let .payInvoiceHandled(
        success, preimage, feesSat, error
    ):
    return [
            "type": "payInvoiceHandled",
            "success": success,
    ...
    return [
            "type": "listTransactionsHandled",
        ]
    
    case .getBalanceHandled:  
    return [
            "type": "getBalanceHandled",
        ]   
    }    
}
...
static func asSdkEvent(sdkEvent: [String: Any?]) throws -> SdkEvent {
    let type = sdkEvent["type"] as! String
        ...
        if (type == "nwc") {
                guard let _eventId = sdkEvent["eventId"] as? String else {
                    throw SdkError.Generic(message: errMissingMandatoryField(fieldName: "eventId", typeName: "SdkEvent"))
                }
                guard let detailsTmp = sdkEvent["details"] as? [String: Any?] else {
                    throw SdkError.Generic(message: errMissingMandatoryField(fieldName: "details", typeName: "SdkEvent"))
                }
            ...
            return SdkEvent.nwc(eventId: _eventId, details: _details)       
        }    
