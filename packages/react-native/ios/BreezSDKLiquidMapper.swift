...
enum BreezSDKLiquidMapper {
    ...
    static func asPrepareSendRequest(prepareSendRequest: [String: Any?]) throws -> PrepareSendRequest {
        ...
        if hasNonNilKey(data: prepareSendRequest, key: "payerNote") {
            guard let payerNoteTmp = prepareSendRequest["payerNote"] as? String else {
                throw SdkError.Generic(message: errUnexpectedValue(fieldName: "payerNote"))
            }
            payerNote = payerNoteTmp
        }

        return PrepareSendRequest(destination: destination, amount: amount, payerNote: payerNote)
    }
    ...
    static func dictionaryOf(prepareSendRequest: PrepareSendRequest) -> [String: Any?] {
        return [
            ...
            "payerNote": prepareSendRequest.payerNote == nil ? nil : prepareSendRequest.payerNote,
        ]
    }
