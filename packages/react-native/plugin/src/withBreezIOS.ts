...
import { withDangerousMod, withEntitlementsPlist, withPodfile, withXcodeProject } from "@expo/config-plugins"
...
export function withNotificationServiceExtension(config: ExpoConfig, props: NotificationServiceExtensionProps): ExpoConfig {
    const appExtensions = (config.extra?.eas?.build?.experimental?.ios?.appExtensions ?? []) as EASAppExtension[]
    ...
    config = withDangerousMod(config, [
        ...
        (config) => {
            ...
            const swiftSource = path.join(__dirname, "./../ios/BreezSDKLiquidNotificationService.swift")
            const swiftDest = path.join(extensionDir, "BreezNotificationService.swift")
    ...
    config = withPodfile(config, (config) => {
        ...
        podFile.contents = podFile.contents.replace(/end[\s]*$/, `end\n${NOTIFICATION_SERVICE_PODS(targetName)}`)
        return config
    })
