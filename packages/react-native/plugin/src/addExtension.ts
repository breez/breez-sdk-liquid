...
export function addExtension({ project, targetName, bundleIdentifier }: AddExtensionProps) {
    ...
    for (const [, config] of Object.entries(existing)) {
        ...
        if ((config as any).buildSettings.PRODUCT_BUNDLE_IDENTIFIER && (config as any).buildSettings.PRODUCT_BUNDLE_IDENTIFIER === bundleIdentifier) {
            return false
        }
    }
    ...
    // Add Extension files
    const extensionGroup = project.addPbxGroup([], targetName, targetName, '"<group>"')
    const notificationServiceSwiftFile = project.addFile("BreezNotificationService.swift", extensionGroup.uuid, {
    ...
    const extensionSourcesBuildPhase = project.addBuildPhase([], "PBXSourcesBuildPhase", "Sources", targetUuid)
    extensionSourcesBuildPhase.buildPhase.files.push(notificationServiceSwiftFile.uuid)
    ...
    addToPbxCopyfilesBuildPhase(project, productFile, "Embed Foundation Extension")
...
function addToPbxCopyfilesBuildPhase(project: XcodeProject, file: any, name: string) {
    const sources = project.buildPhaseObject("PBXCopyFilesBuildPhase", name || "Copy Files", file.target)
