...
export function logPrefix(): string {
    return `› ${bold("[@breeztech/breez-sdk-liquid-react-native/expo]")}`
}
...
/** Reads the version of a package as the app actually installed it, so that the Gradle
 *  coordinates for Expo's precompiled aars can never drift from the JS dependency. */
export function resolvePackageVersion(projectRoot: string, packageName: string): string | undefined {
    try {
