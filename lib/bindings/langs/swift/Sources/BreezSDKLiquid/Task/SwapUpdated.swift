...
class SwapUpdatedTask : TaskProtocol {
    ...
    func onShutdown() {
        ...
        self.displayPushNotification(title: notificationTitle, body: notificationBody, logger: self.logger, threadIdentifier: Constants.NOTIFICATION_THREAD_REPLACEABLE)
    }
    ...
    func notifySuccess(payment: Payment) {
        if !self.notified {
            ...
            self.notified = true
            self.displayPushNotification(title: String(format: notificationTitle, payment.amountSat), logger: self.logger, threadIdentifier: Constants.NOTIFICATION_THREAD_DISMISSABLE)
        }
    }
    ...
    func notifyPaymentWaitingFeeAcceptance(payment: Payment) {
        if !self.notified {
            ...
            self.notified = true
            self.displayPushNotification(title: notificationTitle, body: notificationBody, logger: self.logger, threadIdentifier: Constants.NOTIFICATION_THREAD_DISMISSABLE)
        }
    }
}
