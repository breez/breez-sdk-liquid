...
class InvoiceRequestTask : ReplyableTask {
    ...
    override func start(liquidSDK: BindingLiquidSdk, pluginConfigs: PluginConfigs) throws {
        ...
        do {
            request = try JSONDecoder().decode(InvoiceRequestRequest.self, from: payload.data(using: .utf8)!)
        } catch let e {
        ...
        do {
            ...
            replyServer(encodable: InvoiceRequestResponse(invoice: createBolt12InvoiceRes.invoice), replyURL: request!.reply_url)
        } catch let e as PaymentError {
            logger.log(tag: TAG, line: "failed to process invoice request: \(e)", level: "ERROR")
            let error = e.localizedDescription
            notifyError(request: request, error: error)
        } catch let e {
            logger.log(tag: TAG, line: "failed to process invoice request: \(e)", level: "ERROR")
            notifyError(request: request, error: "Failed to process invoice request")
        }
    }

    func notifyError(request: InvoiceRequestRequest?, error: String) {
        if request != nil {
            replyServer(encodable: InvoiceErrorResponse(error: error), replyURL: request!.reply_url)
        }
        displayPushNotification(title: failNotificationTitle, logger: logger, threadIdentifier: Constants.NOTIFICATION_THREAD_REPLACEABLE)
    }
}
