...
use crate::{
    plugin::{events::PluginEventEmitter, storage::PluginStorage},
    BindingLiquidSdk,
};
...
impl breez_sdk_liquid::prelude::Plugin for WasmPlugin {
    ...
    async fn on_start(
        ...
        event_emitter: breez_sdk_liquid::plugin::PluginEventEmitter,
    ) {
        ...
        self.plugin.on_start(
            ...
            PluginStorage::new(storage),
            PluginEventEmitter::new(event_emitter),
        );
    }
...
const PLUGIN_INTERFACE: &'static str = r#"export interface Plugin {
    id: () => string;
    onStart: (sdk: BindingLiquidSdk, storage: PluginStorage, eventEmitter: PluginEventEmitter) => void;
    onStop: () => void;
}"#;
