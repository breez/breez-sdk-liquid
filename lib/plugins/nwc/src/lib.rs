                    ...
                    tokio::select! {
                        ...
                        Some(_) = Self::min_refresh_interval(&mut maybe_expiry_interval) => {
                            ...
                            for name in result.deleted {
                                thread_ctx.event_manager.notify(NwcEvent { event_id: None, connection_name: None, details: NwcEventDetails::ConnectionExpired { name } }).await;
                            }
                            for name in result.refreshed {
                                thread_ctx.event_manager.notify(NwcEvent { event_id: None, connection_name: None, details: NwcEventDetails::ConnectionRefreshed { name } }).await;
                            }
                        }
