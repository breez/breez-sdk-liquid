...
impl LiquidSdk {
    ...
    fn notify_event_listeners(&self, e: LiquidSdkEvent) -> Result<()> {
        self.event_manager.notify(e);
        Ok(())
    }

    pub fn add_event_listener(&self, listener: Box<dyn EventListener>) -> LiquidSdkResult<String> {
        Ok(self.event_manager.add(listener)?)
    }

    pub fn remove_event_listener(&self, id: String) -> LiquidSdkResult<()> {
        self.event_manager.remove(id);
        Ok(())
    }
    ...
    pub(crate) fn try_handle_receive_swap_update(
        ...
        Ok(self.emit_payment_updated(payment_id)?)
    }
    ...
    fn emit_payment_updated(&self, payment_id: Option<String>) -> Result<()> {
    ...
    pub fn sync(&self) -> Result<()> {
        ...
        self.notify_event_listeners(LiquidSdkEvent::Synced)?;
        Ok(())
    }
