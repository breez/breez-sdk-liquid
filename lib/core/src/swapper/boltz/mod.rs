...
pub struct BoltzSwapper<P: ProxyUrlFetcher> {
    ...
    liquid_electrum_client: LiquidClient,
    bitcoin_electrum_client: BitcoinClient,
    proxy_url: Arc<P>,
...
impl<P: ProxyUrlFetcher> BoltzSwapper<P> {
    pub fn new(config: Config, proxy_url: Arc<P>) -> Result<Self, SdkError> {
        ...
        Ok(Self {
            ...
            client: OnceLock::new(),
            config: config.clone(),
            liquid_electrum_client: LiquidClient::new(&config)?,
            bitcoin_electrum_client: BitcoinClient::new(&config)?,
